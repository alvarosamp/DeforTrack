...
\textcolor{red}{Among the lightweight variants, YOLOv8n was the fastest and least energy-intensive checkpoint, reaching $0.0120 \pm 0.0001$~s/image, 83.39 FPS, and 0.263 GPU J/image. YOLOv11n had the smallest checkpoint (5.75 MB), but was slower at $0.0156 \pm 0.0004$~s/image and 64.00 FPS. Thus, checkpoint size alone did not determine workstation latency. YOLOv8s retained substantially higher segmentation accuracy than both nano variants while remaining close to their throughput, making it the strongest accuracy--efficiency compromise in this experiment.}

\textcolor{red}{Mask R-CNN remained competitive in region segmentation but incurred higher resource use. ResNet-101 FPN 3x was the best Mask R-CNN variant by IoU ($77.92~\%$), with $0.0483 \pm 0.0009$~s/image, 20.72 FPS, 1949.74 MB peak process RAM, and 2.833 GPU J/image. ResNet-50 C4 3x was the fastest Mask R-CNN variant at $0.0310 \pm 0.0002$~s/image and 32.25 FPS, but its IoU was lower at $75.92~\%$. U-Net had the lowest peak process RAM (985.39 MB) and reached 64.77 FPS, yet its IoU ($49.97~\%$) and Boundary IoU ($20.93~\%$) were the lowest in the internal benchmark. It is therefore retained as a compact semantic-segmentation baseline rather than the preferred model for precise forest-boundary mapping.}
...
\textcolor{red}{To assess cross-dataset transfer without adapting the trained weights, we evaluated the final checkpoints on two external datasets used exclusively for testing. Forest Aerial Images~\cite{quadeer2024augmentedforest} contributed all 5,108 available paired RGB images and binary forest masks; this local benchmark did not provide a separate test partition, so the complete paired set was used as one external evaluation set and was never used for DeforTrack training or model selection. Its native $256\times256$ RGB images match the U-Net input size. For YOLO and Mask R-CNN, predictions were mapped to the native mask dimensions before metric calculation; for U-Net, images and masks were evaluated at $256\times256$.}
...
\textcolor{red}{Table~\ref{t6} shows that external performance depends strongly on the visual and sensor characteristics of each dataset. U-Net obtained $60.58~\%$ IoU and $70.36~\%$ Dice/F1 on Forest Aerial Images, exceeding its DeforTrack IoU. This difference should not be interpreted as superior generalization: Forest Aerial Images has the same native resolution as the U-Net input and contains comparatively broad, visually homogeneous forest regions, whereas downsampling DeforTrack from $640\times640$ to $256\times256$ can suppress narrow and fragmented boundaries. Different class distributions and scene difficulty also prevent direct ranking across datasets. On the more dissimilar Sentinel-2 benchmark, YOLOv11n achieved $47.96~\%$ IoU and $58.59~\%$ Dice/F1, while the stronger domain shift reduced the performance of the remaining models. These results demonstrate partial transfer to visually compatible scenes but do not establish geographic, biome-specific, or sensor-invariant generalization. Figure~\ref{f4} shows representative samples from both external benchmarks.}
...
\textcolor{red}{Although the quantitative and qualitative results indicate that YOLO-based models achieved the best trade-off between segmentation accuracy and computational efficiency on the final test set, several limitations constrain their interpretation. Segmentation errors are more likely under shadows, strong illumination variation, low contrast, sparse or dry vegetation, and mixed land-cover patterns. The most difficult cases occur near visually ambiguous forest/non-forest transitions. Fragmented landscapes, exposed soil, agricultural fields, roads, shadows, and low vegetation density can resemble forest canopy or partially preserved vegetation, producing false positives; conversely, sparse, low, or degraded forest can be omitted, producing false negatives. These failure modes show that aggregate metrics alone do not guarantee reliable behavior across all vegetation structures.}

\textcolor{red}{The most consequential dataset limitation is the absence of complete image-level geographic metadata in the final export. DeforTrack was designed for Brazilian deforestation monitoring and contains a predominance of Brazilian forest scenarios, but it also combines private and public sources from different regions. Because source, coordinates, acquisition date, and biome were not consistently preserved, every sample cannot be assigned reliably to a Brazilian biome, and an independently stratified spatial evaluation cannot be constructed retrospectively. Consequently, the qualitative panels represent distinct visual forest and land-cover scenarios, not confirmed examples of specific Brazilian biomes; the current results must not be used to claim biome-specific performance.}
